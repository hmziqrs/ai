...
name: z-proflow
description: >-
  State-file-driven Codex orchestration loop — the depth-first variant:
  ...
  glm-5.3 via `zai_pro` — main-tier reasoning for judgment-heavy
...
via `zai_pro`; the main thread alone uses the host model, and only
...
| State decider (continue/stop, compaction) | GLM 5.3 | `zai_pro` | Honesty terminus with main-tier depth; input is compact |
| Implementation — complex (concurrency, architecture, subtle bugs) | GLM 5.3 | `zai_pro` | Judgment-heavy generation — the reason this flow exists |
| Implementation — straightforward (CRUD, boilerplate, simple UI) | GLM 5.3 | `zai_pro` | One tier everywhere; gates + fix loop are the backstop |
| Mechanical gates (`cargo test`, builds, linters) | Direct Bash, no sub-agent | — | Objective truth, no model needed |
| Judge (code audit + verdicts) | GLM 5.3 | `zai_pro` | Rule judgment needs depth — main-tier audit verdicts |
| Committer (stage, message, commit) | GLM 5.3 | `zai_pro` | Verifiable via `git log` |

Dispatch by sub-agent type, never by prompt stamp. `zai_pro` pins
...
Init + survey (main thread): STATE.md + routing map + project rules
   ...
   │      │ pass                                     │          │
   │      ▼                                          │  fix     │
   │  Committer (zai_pro)                        │  rounds  │
   │      ▼                                          │          │
   │  Judge (zai_pro): code audit of             │          │
...
always `zai_pro`, straightforward and complex alike; the type
...
When an area's gates pass, spawn the committer — a `zai_pro` agent —
...
per-area cycle. Spawn a fresh `zai_pro` agent (glm-5.3 pinned),
...
Spawn the decider — a `zai_pro` agent (glm-5.3 pinned) —
...
spawn a fresh fixer with the FULL Phase 1 template — zai_pro
