...
name: z-workflow
description: >-
  State-file-driven Codex orchestration loop for complex multi-phase engineering
...
| State decider (continue/stop, compaction) | GLM 5.3 | `zai_pro` | Honesty terminus; input is compact so the cost is trivial |
| Router (repo survey → routing map) | GLM 5.3 Flash | `zai_flash` | Table lookup + structured output; a misroute dies at the gates |
| Implementation — complex (concurrency, architecture, subtle bugs) | GLM 5.3 | `zai_pro` | Judgment-heavy generation |
...
| Judge (code audit + verdicts + vision FAIL review) | GLM 5.3 | `zai_pro` | Rule judgment needs depth |
...
pin their own model — `zai_flash` runs glm-5.3-flash, `zai_pro`
...
(`worker` inherits the host model) and stamp the model name as
...
```
Init STATE.md ──► Router (zai_flash): routing map + rules
                        │
                        ▼  (per area, routing-map order, dependencies first)
   ┌────────────────────────────────────────────────────────────┐
   │  Implementer (zai_pro | zai_flash) ◄────┐          │
   │      │ self-runs gates                          │          │
   │      ▼                                          │          │
   ...
   │      ▼                                          │          │
   │  Judge (zai_pro): code audit of this        │          │
   ...
   Vision (zai_vision, run-level: ONE pass over the whole UI; skip
          ...
          ▼
   Judge (zai_pro): reviews vision FAIL evidence + any commits not
          yet judged
          ▼
   Decider (zai_pro): apply POLICY, write HEADER
...
2. Write a concise in-chat progress plan with one item per phase. Update it as rounds
   pass; on a long run it is the user's window into the work.

## Phase 1 — Router (GLM 5.3 Flash)

Spawn a `zai_flash` agent with the state file path: survey the repo
...
(`straightforward` → zai_flash | `complex` → zai_pro), depends-on, has-UI.
...
- **Visual tooling** is capability-derived, never assumed: use the available
  ...
  is present in the session. The router never assigns visual tooling to an
  implementation area; the run-level `zai_vision` agent owns it.

## Phase 2 — Implementers (one area at a time)

Spawn one agent per area, in routing-map order (dependencies first) —
`zai_flash` for straightforward areas, `zai_pro` for complex
...
router's call, not yours). Spawn a `zai_vision` verification agent
that:
...
yet judged). Spawn a fresh `zai_pro` agent (glm-5.3 pinned — do not
...
built-in fallback only — omit it when dispatching `zai_pro`. Prompt core:
...
Spawn the decider — a `zai_pro` agent (glm-5.3 pinned) — after every
