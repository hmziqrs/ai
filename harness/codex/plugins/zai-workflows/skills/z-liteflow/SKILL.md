...
name: z-liteflow
description: >-
  Lightweight Codex orchestration loop for small, single-domain tasks — the
...
| Spawn the implementer (`zai_flash`) and a fresh auditor (`zai_explorer`) each round | Fix code to make an audit pass — that's a fix round's job |
...
| Implementation + fix rounds | `zai_flash` (GLM 5.3 Flash) |
| Audit | `zai_explorer` (GLM 5.3 Flash), fresh agent every round |
| Vision (only if the user asks) | `zai_vision` (GLM 5.3 Flash) |

The custom types are user-scope agents (~/.codex/agents/). The built-ins
`worker` / `explorer` inherit the host model instead, so they are
...
pairs — `worker` wherever `zai_flash` is named, `explorer`
...
```
Scope (main thread) ──► Baseline gate run ──► Implementer (zai_flash)
   ──► Your gate re-run ─┬─ red ── counts as this round's verdict ──┐
                         └─ green ─► Auditor (zai_explorer, fresh) │
                                       ├── clean ─────────► Report  │
                                       └── blocker/major ───────────┤
                                                                    ▼
                          Fix round (zai_flash: resumed implementer
...
Dispatch to a `zai_flash` agent — the type pins the model; the stamp
...
repo — paste that), then spawn a fresh `zai_explorer` agent every round
...
sideways, spawn a fresh fixer (`zai_flash`): the full Phase 2 template
...
same checklist. The vision agent (dispatch as `zai_vision`) owns app
...
`/tmp/z-liteflow/<task-slug>/` with stable names. Use the available browser
tools for web flows and `unified-computer-use` for desktop flows; only use a
mobile skill when it is actually present in the session. Say in
