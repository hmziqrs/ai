...
name: z-flashflow
description: >-
  State-file-driven Codex orchestration loop — the all-flash variant of
  ...
  via `zai_flash` — no main-tier model anywhere — so multi-phase
  ...
  implementation or audit depth demands GLM 5.3. Trigger phrases: flash
...
Principle: **Flash everywhere, by design.** Every sub-agent runs
glm-5.3-flash via `zai_flash`; the main thread alone uses the host
model, and only to sequence. Flash judgment is compensated mechanically —
objective gates, fresh auditors every pass, fix rounds, the decider's
POLICY checkpoint. When depth is the bottleneck, that is an escalation to
z-workflow (see Escalation), never a mid-run model swap.
...
| State decider (continue/stop, compaction) | GLM 5.3 Flash | `zai_flash` | Input is compact; honesty comes from POLICY rules, not model depth |
| Implementation — complex (concurrency, architecture, subtle bugs) | GLM 5.3 Flash | `zai_flash` | Gates + fix loop catch failures; stalling complex areas escalate |
| Implementation — straightforward (CRUD, boilerplate, simple UI) | GLM 5.3 Flash | `zai_flash` | Gates + fix loop catch failures objectively |
| Mechanical gates (`cargo test`, builds, linters) | Direct Bash, no sub-agent | — | Objective truth, no model needed |
| Judge (code audit + verdicts + vision FAIL review) | GLM 5.3 Flash | `zai_flash` | Checklist audit; over-flagging is absorbed by fix rounds + re-audit |
| Vision evidence capture | GLM 5.3 Flash | `zai_vision` | Screenshot, browser, and computer-use evidence |
| Committer (stage, message, commit) | GLM 5.3 Flash | `zai_flash` | Verifiable via `git log` |

Dispatch by sub-agent type, never by prompt stamp. `zai_flash` pins
glm-5.3-flash, so no role depends on inheriting the main thread's model.
...
`worker` (which inherits the host model) and stamp
`Model: GLM 5.3 Flash` as the first line of every prompt, noting in the
...
Init + survey (main thread): STATE.md + routing map + project rules
   ...
   ┌────────────────────────────────────────────────────────────┐
   │  Implementer (zai_flash) ◄──────────────────┐          │
   │      │ self-runs gates                          │          │
   │      ▼                                          │          │
   │  Gates (main thread) ── fail───────────────────►│          │
   │      │ pass                                     │          │
   │      ▼                                          │  fix     │
   │  Committer (zai_flash)                      │  rounds  │
   │      ▼                                          │          │
   │  Judge (zai_flash): code audit of           │          │
   │  this area's commits ── findings────────────────┴──────────┘
   │      │ clean
   └──────┼─────────────────────────────────────────────────────┘
          ▼ all areas clean
   Vision (zai_vision, run-level: ONE pass over the whole UI; skip
          if the routing map marks no UI areas)
          ▼
   Judge (zai_flash): reviews vision FAIL evidence + any commits
          not yet judged
          ▼
   Decider (zai_flash): apply POLICY, write HEADER
...
5. Write a concise in-chat progress plan with one item per phase. Update it as rounds
...
- **Visual tooling** is capability-derived, never assumed: use available
  browser tools for web flows and the `unified-computer-use` plugin for
  native-desktop flows. Only use Android or iOS tooling when a matching skill
  is present. Never assign visual tooling to implementation areas; the
  run-level `zai_vision` agent owns it.

## Phase 1 — Implementers (one area at a time)

Spawn one agent per area, in routing-map order (dependencies first) —
always `zai_flash`, straightforward and complex alike; the type
...
When an area's gates pass, spawn the committer — a `zai_flash` agent —
...
not mid-run). Spawn a `zai_vision` verification agent
that:

1. Uses the available browser tools for web verification and the
   `unified-computer-use` plugin for native-desktop verification. If the
   required capability is unavailable, records that gap rather than inventing
   a substitute.
...
yet judged). Spawn a fresh `zai_flash` agent (glm-5.3-flash pinned),
read-only for product code — its only write is appending to STATE.md. One
agent, one pass; no triage layer, no split verdicts. Flash auditors
over-flag — do not soften findings to compensate; false positives are
absorbed by fix rounds and re-audit. The template's `Model:` stamp is the
built-in fallback only — omit it when dispatching `zai_flash`.
Prompt core:
...
Spawn the decider — a `zai_flash` agent (glm-5.3-flash pinned) —
...
spawn a fresh fixer with the FULL Phase 1 template — zai_flash
...
## Escalation to z-workflow

All-flash is a bet that gates and fix rounds catch what model depth would
have prevented. Exit the bet when:

- A `complex`-tagged area stalls — two fix rounds with no shrinking
  findings on the same issue.
- The judge repeatedly contradicts green gates, or contradicts itself
  across rounds.
- The user asks for main-tier audit depth.

Stop cleanly: the decider writes the run summary, report honestly what is
done and what is open, and name z-workflow as the hand-off. Half-done
work stays on disk, listed in the report; revert only on request.
