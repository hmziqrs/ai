...
glm-5.3-flash via `general-flash`; the main thread alone uses the host
...
| Judge (code audit + verdicts + vision FAIL review) | GLM 5.3 Flash | `general-flash` | Checklist audit; over-flagging is absorbed by fix rounds + re-audit |
...
Dispatch by sub-agent type, never by prompt stamp. `general-flash` pins
...
Init + survey (main thread): STATE.md + routing map + project rules
   ...
   │      ▼                                          │          │
   │  Judge (general-flash): code audit of           │          │
   ...
   Vision (general-flash, run-level: ONE pass over the whole UI; skip
          ...
          ▼
   Judge (general-flash): reviews vision FAIL evidence + any commits
          not yet judged
          ▼
   Decider (general-flash): apply POLICY, write HEADER
...
yet judged). Spawn a fresh `general-flash` agent (glm-5.3-flash pinned),
...
built-in fallback only — omit it when dispatching `general-flash`.
Prompt core:
